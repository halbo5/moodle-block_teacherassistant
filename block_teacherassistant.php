...
class block_teacherassistant extends \block_base {
    ...
    public function get_content() {
        ...
        if (!has_capability('moodle/course:create', $systemcontext)) {
            ...
            $this->content->text .= html_writer::link($createurl,
                    get_string('text_createcourse', 'block_teacherassistant'),
                    array('style' => 'color:inherit'));
            $this->content->text .= html_writer::end_tag('p');
            if ($deleteid != 0) {
                $this->content->text .= html_writer::start_tag('p');
                $this->content->text .= html_writer::link($deleteurl,
                        get_string('text_deletecourse', 'block_teacherassistant'));
                $this->content->text .= html_writer::end_tag('p');
            }
            if ($cohortid != 0) {
                $this->content->text .= html_writer::start_tag('p');
                $this->content->text .= html_writer::link($cohorturl,
                        get_string('text_findcohort', 'block_teacherassistant'));
                $this->content->text .= html_writer::end_tag('p');
            }
            if ($rightsid != 0) {
                $this->content->text .= html_writer::start_tag('p');
                $this->content->text .= html_writer::link($rightsurl,
                        get_string('text_modifyrights', 'block_teacherassistant'));
                $this->content->text .= html_writer::end_tag('p');
            }
            if ($learningspaceid != 0) {
                $this->content->text .= html_writer::start_tag('p');
                $this->content->text .= html_writer::link($learningspaceurl,
                        get_string('text_learningspace', 'block_teacherassistant'));
                $this->content->text .= html_writer::end_tag('p');
            }
            if ($trainingurl != '') {
                $this->content->text .= html_writer::start_tag('p');
                $this->content->text .= html_writer::link($trainingurl,
